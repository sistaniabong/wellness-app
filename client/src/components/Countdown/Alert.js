...
function Alert({reminders}) {
    ...
    const showModal = () => {
     ...
     const [updateCompleteReminder, { error, data }] = useMutation(UPDATE_COMPLETE_REMINDER);
    ...
    const hideModal = () => {
     ...
     const handleClose = async (reminderId) =>{

        console.log("handleClose")
        try {
          console.log('init update reminder')
          const { data } = await updateCompleteReminder({
   ...
   return (
        <div style={{ visibility: status ? 'hidden' : 'visible'}} show={show} onHide={hideModal}>
    {reminders.map((reminder) => (
  <Modal.Dialog> 
  <Modal.Header closeButton>
    <Modal.Title style={{ color: 'turquoise'}}>{reminder.title}</Modal.Title>
  </Modal.Header>

  <Modal.Body>
    <p style={{ color: 'black'}}>{reminder._id, 'This is your reminder!'}</p>
  </Modal.Body>

  <Modal.Footer>
    <Button variant="primary" 
    onClick={() => handleClose(reminder._id)}
    className="btn-floating btn-large red lighten-2" style={{width:'90px', height:'50px'}}>Complete</Button>
